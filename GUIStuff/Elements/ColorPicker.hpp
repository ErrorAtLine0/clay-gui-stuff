...
template <typename T> class ColorPicker : public Element {
    public:
        void update(UpdateInputData& io, T* newData, bool newSelectAlpha, const std::function<void()>& elemUpdate) {
            ...
            CLAY({
                .layout = {
                    .sizing = {.width = CLAY_SIZING_GROW(0), .height = CLAY_SIZING_FIXED(bb.dim.x()) }
                },
                ...
                if(selection.clicked && data) {
                    ...
                    modifyingSv = SCollision::collide(SCollision::AABB<float>(bb.pos, bb.pos + Vector2f{svSelectionAreaSize, svSelectionAreaSize}), io.mouse.pos);
                ...
                if(selection.held && modifyingSv) {
                    float svSelectionAreaSize = bb.dim.x() - (BAR_GAP + BAR_WIDTH);
                    Vector2f newSv = cwise_vec_clamp<Vector2f>((io.mouse.pos - bb.pos) / svSelectionAreaSize, Vector2f{0.0f, 0.0f}, Vector2f{1.0f, 1.0f});
                    savedHsv.y() = newSv.x();
    ...
    private:
        ...
        float get_sv_selection_area_size() {
            return bb.dim.x() - (BAR_GAP + BAR_WIDTH);
        }

        Vector2f get_hue_bar_pos() {
            return {bb.pos.x() + get_sv_selection_area_size() + BAR_GAP, bb.pos.y()};
        }
        ...
        Vector2f get_alpha_bar_pos() {
            return {bb.pos.x(), bb.pos.y() + get_sv_selection_area_size() + BAR_GAP};
        }

        Vector2f get_alpha_bar_dim() {
            return {bb.dim.x(), BAR_WIDTH};
        }
